...
def create_folds(data, labels, k=6):
    ...
    for fold_size in fold_sizes:
        start_idx, end_idx = current, current + fold_size
        fold_indices = indices[start_idx:end_idx]
        fold_data = [data[i] for i in fold_indices]
        fold_labels = [labels[i] for i in fold_indices]
        folds.append((fold_data, fold_labels))
        current = end_idx
        
        print(f"Fold {len(folds)}: {len(fold_data)} elementos")
...
if __name__ == '__main__':
  ...
  #Com cross validation
    ...
    n_neurons = [20,30,40,50,60,70,80,100,120,140,160,180,200]
    result = []
    time_history = []
    ...
    for i in n_neurons:    
        ...
        print()
        print(f'========={i} NEURONIOS=========')
        mean_accuracy = validacao_cruzada(inputs_train, outputs_train, k=5, epochs=110, learning_rate=0.04)
        result.append(mean_accuracy)
        time_history.append(time.time() - train_time)
        
    
    print()
    for i, n in enumerate(n_neurons):
        print(f'{n} neuronios -> Acuracia {result[i]}. Tempo: {time_history[i]:.2f} s')
    
    print(f'Tempo Total: {sum(time_history):.2f} s')
